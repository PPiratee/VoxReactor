...
namespace PPirate.VoxReactor
{
    public class UIManager
    {
        ...
        public static UIDynamicTabBar CreateTabBar(List<UIPannel> menuItems, Action<UIPannel> selectTab, int columns = -1,
            MVRScript script = null, bool invokeDefault = false, float width = 1f, float itemHeight = 50f)
        {
            if (script == null) script = UIManager.script;
            if (columns == -1 || columns >= menuItems.Count) columns = menuItems.Count;
            float rowCount = Mathf.Ceil((float)menuItems.Count / columns);
            ...
            for (int i = 0; i < menuItems.Count; ++i)
            {
                ...
                uid.buttons.Add(buttonButton);
                Text buttonText = buttonTransform.Find("Text").GetComponent<Text>();
                buttonText.text = menuItems[i].tabLabel;
